  ...
  try {
    ...
    const content = await page.evaluate(() => {
      ...
      const facts = getText("p, li, dd, td")
        .filter(
          (t) =>
            ...
            /(first|largest|oldest|founder|created|invented|launched|developed|introduced)/i.test(
              t,
            ),
        )
        .slice(0, 10);
      ...
      const specific = getText("p, li, h2, h3, blockquote")
        .filter(
          (t) => !/(cookie|privacy|subscribe|newsletter|sign up)/i.test(t),
        )
        .slice(0, 15);
...
DO NOT repeat any of these user links: ${links.join(", ") || "(none)"}`,
    ...
    const seeds: ResearchSite[] = [...explicitSeeds, ...additionalSeeds];
